...
const Doctor = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (storedData?.doctor) {
        ...
        setName(cleanName);
        setDoctorId(storedData.doctor._id); // ✅ IMPORTANT
      }
    } catch (err) {
  ...
  <button
    ...
    onClick={() => navigate(`/doctor/${doctorId}/allpatients`)}
  >
    👥 All Patients
  </button>
  ...
  <button
    className="btn-primary"
    onClick={() => navigate(`/doctor/${doctorId}/addpatient`)}
  >
    ➕ Add Patient
  </button>
  ...
  <button
    className="btn-primary"
    onClick={() => navigate(`/doctor/${doctorId}/healthcamps`)}
  >
    🏥 Health Camps
  </button>
